          ...
          <div class="product-card">
            <div class="product-details p-4">
              <div class="d-flex justify-content-between align-items-start mb-3">
                <span class="auction-badge position-relative top-0 start-0 ${badge} px-3 py-1 rounded-pill" style="font-size:0.7rem; box-shadow:none;">${p.status}</span>
                <span class="text-muted small"><i class="bi bi-calendar me-1"></i>${p.requestedDate}</span>
              </div>
              <h5 class="product-title text-truncate fw-bold mb-2" title="${p.productName}">${p.productName}</h5>
              <p class="small text-muted mb-3"><i class="bi bi-tag-fill me-1"></i> ${p.category} <span class="mx-2">•</span> ${p.condition}</p>
              ...
              <div class="d-flex justify-content-between align-items-center mt-auto pt-3 border-top" style="border-color: rgba(255,255,255,0.05) !important;">
                <div>
                  <div class="small text-muted mb-1">Max Budget</div>
                  <div class="bid-price fs-4">₹${(p.maxBudget || p.budget || 0).toLocaleString('en-IN')}</div>
                </div>
                <a href="wanted-product-details.html?id=${p.id}" class="btn btn-outline-premium btn-sm px-3 py-2">View Request</a>
              </div>
            </div>
          </div>
        </div>
